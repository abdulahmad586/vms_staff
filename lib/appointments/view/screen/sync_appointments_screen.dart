...
class _SyncAppointmentsState extends State<SyncAppointments> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar( title: Text('SYNCHRONIZE MEETINGS', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: AppColors.primaryColor, fontFamily: 'Iceberg')),),

      body: BlocProvider<AppointmentsCubit>(
        create: (context)=>AppointmentsCubit(AppointmentState(fromTime: DateTime.now(), toTime:times['to'], pageSize: 500), refreshController: refreshController, ),
              ...
              child:Container(
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: state.list == null || state.list!.isEmpty ? EmptyResult(message: "No meetings found",loading: state.loading ??false,):
                      ListView.builder(
                        itemCount: state.list!.length,
                        itemBuilder: (_,index)=>MeetingListItem(
                            state.list![index],
                            trailing: Column(
                              children: [
                                Icon(Icons.people, color: Theme.of(context).textTheme.labelSmall?.color?.withAlpha(100),),
                                Text((state.list![index].id).toString(), style: Theme.of(context).textTheme.labelSmall?.copyWith(color: Theme.of(context).primaryColor),),
                              ],
                            )
                        ),
                      ),
                      ...
                      onPressed: ()async{
                        if(state.list!=null && state.list!.isNotEmpty){
                          ...
                          try{
                            ...
                            Alert.message(context,title:"Synced!",message: "Calendar synced successfully", error: false, close: (){
                              Navigator.pop(context);
                            });
                          }catch(e){
                            Alert.message(context,message: "Unable to sync to device calendar, please make sure you have granted permission to the app.", error: true);
                            setState(() {
                              loading=false;
                            });
                          }
                          ...
                          Alert.toast(context, message:"Can't sync an empty list");
                        }
                      },
                    )
                  ],
